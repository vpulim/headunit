...
QXineWidget::~QXineWidget()
{
  ...
  qWarning("Shut down xine engine");
 ...
 // KApplication::kApplication()->lock(); /* lock QT event thread */
  ...
  switch (xineEvent->type)
  {
    ...
    case XINE_EVENT_UI_SET_TITLE:  /* set new title */
	 ...
	 qWarning("new mrl: %s",vw->newMrlReference.latin1());
...
void QXineWidget::slotStartPlayback()
{
...
bool QXineWidget::GetPosition(long *pos, long *len)
{
  int pos_stream;
  *pos = 0;
  *len = 0;
  ...
  int p, l;
  if(!xine_get_pos_length (xineStream, &pos_stream, &p, &l)) {
    return false;
  }
  *pos = p;
  *len = l;
...
void QXineWidget::GetScreenshot(uchar*& rgb32BitData, int& videoWidth, int& videoHeight, double& scaleFactor) const
{
